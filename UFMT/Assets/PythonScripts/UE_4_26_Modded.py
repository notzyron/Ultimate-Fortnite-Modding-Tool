...
def create_anim_blueprint(anim_bp_name, skeleton_asset_name, template_path="/Game/AnimBP_Template"):
    new_path      = "{}/{}".format(fbx_destination_path, anim_bp_name)
    ...
    if not success:
        unreal.log_error("FAILED — could not duplicate anim blueprint: {}".format(anim_bp_name))
        return

    anim_bp = unreal.load_asset(new_path)

    if not anim_bp:
        unreal.log_error("FAILED — could not load duplicated anim blueprint: {}".format(new_path))
        return
...
for i in range(len(fbx_paths)):
    ...
    if asset_names[i] in physics_mesh_names:
        create_anim_blueprint("{}_AnimBP".format(asset_names[i]), "{}_Skeleton".format(asset_names[i]))
        mesh = unreal.EditorAssetLibrary.load_asset("{}/{}".format(fbx_destination_path, asset_names[i]))
        anim_bp = unreal.EditorAssetLibrary.load_asset("{}/{}_AnimBP".format(fbx_destination_path, asset_names[i]))
        physics_assets = []
        ...
        mesh.set_editor_property("post_process_anim_blueprint", unreal.load_class(None, "{0}/{1}_AnimBP.{1}_AnimBP_C".format(fbx_destination_path, asset_names[i])))
